...
ZWC_FILE="$ZWC_DIR/$LOCATION-$TODAY.txt"
...
if [[ ! -f $ZWC_FILE || $ZWC_FILE != "$ZWC_DIR/$LOCATION-$TODAY.txt" ]]; then
  # Check internet connection
  if $(ping -q -W 1 -c 1 google.com > /dev/null); then
    if [ -f "$ZWC_DIR/$LOCATION-$YESTERDAY.txt" ]; then
      rm "$ZWC_DIR/$LOCATION-$YESTERDAY.txt"
    fi
    echo "Retrieving weather data..."
    curl --silent "wttr.in/$LOCATION?FQ2" > "$ZWC_FILE"
  else
    echo "\033[0;31m No internet connection, weather data not retrieved.\033[0m"
    return
  fi
fi
echo "Today's and tommorow's weather for $LOCATION:"
